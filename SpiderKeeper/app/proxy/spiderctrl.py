...
class SpiderAgent():
    ...
    def __init__(self):
        # 主爬虫服务实例列表
        self.spider_service_instances_main = []
        # 从爬虫服务实例列表
        self.spider_service_instances_subordinate = []

    def regist(self, spider_service_proxy, is_main):
        """
        注册爬虫代理
        ...
        if isinstance(spider_service_proxy, SpiderServiceProxy):
            ...
            if is_main == '1':
                # 将ScrapydProxy(server)添加到spider_service_instances_main列表中
                self.spider_service_instances_main.append(spider_service_proxy)
            else:
                # 将ScrapydProxy(server)添加到spider_service_instances_subordinate列表中
                self.spider_service_instances_subordinate.append(spider_service_proxy)
    ...
    def delete_project(self, project):
        ...
        if project.is_msd == '0':  # 如果是单机爬虫项目
            ...
            for spider_service_instance in self.spider_service_instances_main:
            ...
            for spider_service_instance in self.spider_service_instances_subordinate:
    ...
    def get_spider_list(self, project):
        ...
        :return: 主爬虫的spider对象
        """
        spider_instance_list_subordinate = []
        spider_instance_list_main = []
        ...
        for spider_service_instance in self.spider_service_instances_subordinate:
            ...
            if project.project_name in spider_service_instance.list_projects():
                # 取出该服务器上某个工程名下的所有从爬虫
                spider_instance_list_subordinate = \
                    spider_service_instance.get_subordinate_spider_list(project.project_name)
                break
        # 判断从爬虫服务器有该工程的爬虫
        if spider_instance_list_subordinate:
            for spider_instance, subordinate_spider_name in zip(spider_instance_list_main, spider_instance_list_subordinate):
                # 给每个spider_instance的project.id赋值
                spider_instance.project_id = project.id
                spider_instance.spider_name_subordinate = subordinate_spider_name
            return spider_instance_list_main
        # 判断从爬虫服务器没有该工程的爬虫
        else:
            for spider_instance in spider_instance_list_main:
                # 给每个spider_instance的project.id赋值
                spider_instance.project_id = project.id
            return spider_instance_list_main
    ...
    def start_spider(self, job_instance):
        ...
        if project.is_msd == '0':  # 如果是单机爬虫
            ...
            for leader in self.spider_service_instances_main:
            ...
            # 主爬虫名
            spider_name_main = job_instance.spider_name
            spider_instance = SpiderInstance.query.filter_by(
                project_id=job_instance.project_id, spider_name=spider_name_main).first()
            # 从爬虫名
            spider_name_subordinate = spider_instance.spider_name_subordinate
            # 启动从爬虫服务器启动成功标志
            subordinate_flag = False
            # 从爬虫的job执行列表
            serviec_job_id_subordinate = []
            # 从爬虫运行的服务器列表
            running_on_subordinate = []
            # 遍历从爬虫服务器
            for leader in self.spider_service_instances_subordinate:
                # 启动爬虫, 爬虫启动成功,返回id, 否则返回None
                serviec_job_id = leader.start_spider(project.project_name, spider_name_subordinate)
                # 如果启动成功
                if serviec_job_id:
                    # 标志为True
                    subordinate_flag = True
                    # job_id添加到列表, 为日志获取提供数据
                    serviec_job_id_subordinate.append(serviec_job_id)
                    # 运行的服务器添加到列表, 为日志获取提供数据
                    running_on_subordinate.append(leader.server)
            # 将列表转换为字符串
            serviec_job_id_subordinate_str = ','.join(serviec_job_id_subordinate)
            running_on_subordinate_str = ','.join(running_on_subordinate)
            # 从爬虫服务器至少有一个启动成功,则启动主爬虫服务器
            if subordinate_flag:
                for leader in self.spider_service_instances_main:
                    serviec_job_id = leader.start_spider(project.project_name, spider_name_main)
                    # 如果启动成功
                    if serviec_job_id:
                        job_execution = JobExecution()
                        job_execution.project_id = job_instance.project_id
                        job_execution.service_job_execution_id = serviec_job_id+'>'+serviec_job_id_subordinate_str
                        job_execution.job_instance_id = job_instance.id
                        job_execution.create_time = datetime.datetime.now()
                        job_execution.running_on = leader.server+'>'+running_on_subordinate_str
                        db.session.add(job_execution)
                        db.session.commit()
                        break
    ...
    def cancel_spider(self, job_execution, project_name):
        ...
        """
        # 轮训爬虫代码运行的服务器列表
        for spider_service_instance in self.spider_service_instances_main + self.spider_service_instances_subordinate:
    ...
    def deploy(self, project, egg_path_dict, is_msd):
        ...
        if is_msd == '0':  # 如果是单机爬虫从主爬虫代理中选择一个代理部署爬虫代码
            spider_service_instance = random.choice(self.spider_service_instances_main)
            # 部署单机爬虫
            main_flag = spider_service_instance.deploy(
            ...
            for spider_service_instance in self.spider_service_instances_subordinate:
                if spider_service_instance.deploy(
                        project.project_name, egg_path_dict.get("subordinate")):
                    subordinate_flag = True
            ...
            if subordinate_flag:
                ...
                for spider_service_instance in self.spider_service_instances_main:
                        # 部署主爬虫
                        main_flag = spider_service_instance.deploy(
                                        project.project_name, egg_path_dict.get('main'))
                        # 只要有一个主爬虫部署, 则返回True
                        if main_flag:
                            return True
                        else:
                ...
                return subordinate_flag

    def log_url_main(self, job_execution):
        """
        功能: 获取主爬虫的日志
        ...
        # 主爬虫服务器
        main_service_job_execution_id = service_job_execution_id[0]
        # 爬虫运行的服务器地址
        running_on = job_execution.running_on.split('>')
        main_running_on = running_on[0]
        # 调用主爬虫的日志
        for spider_service_instance in self.spider_service_instances_main:
            if spider_service_instance.server == main_running_on:
                main_log_url = spider_service_instance.log_url(
                    project.project_name, job_instance.spider_name,
                    main_service_job_execution_id)
                return main_log_url

    def log_url_subordinate(self, job_execution):
        """
        功能: 获取从爬虫的日志,只要获取一个
        :param job_execution: job_execution对象
        :return: 返回log的url
        ...
        service_job_execution_id = job_execution.service_job_execution_id.split('>')
        #  从爬虫服务器列表
        subordinate_service_job_execution_id = service_job_execution_id[1].split(',')
        # 爬虫运行的服务器地址
        running_on = job_execution.running_on.split('>')
        subordinate_running_on = running_on[1].split(',')
        # 调用从爬虫的日志
        spider_name_subordinate_obj = SpiderInstance.query.filter_by(
            spider_name=job_instance.spider_name,
            project_id=job_instance.project_id).first()
        spider_name_subordinate = spider_name_subordinate_obj.spider_name_subordinate

        for spider_service_instance in self.spider_service_instances_subordinate:
            for job_execution_id, running_on_ in zip(subordinate_service_job_execution_id, subordinate_running_on):
                if spider_service_instance.server == running_on_:
                    subordinate_log_url = spider_service_instance.log_url(
                        project.project_name, spider_name_subordinate,
                        job_execution_id)
                    return subordinate_log_url
